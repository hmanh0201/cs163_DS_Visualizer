...
void MST::SetCode(const std::vector<std::string>& code) { currentCodeLines = code; }

void MST::PushNewStep(const std::vector<VertexState>& nextNodes, const std::vector<EdgeState>& nextEdges, int codeLine) {
...
void MST::UpdatePositionsAfterDrag() {
...
void MST::GenerateInit(int n, const std::vector<MEdge>& edges) {
    AnimationNodes.clear(); AnimationEdges.clear(); stepCodeLines.clear();
    internalNodes.clear(); internalEdges.clear();
    ...
    for (int i = 0; i < n; i++) {
        ...
        vState.radius = 28.0f; vState.opacity = 1.0f; vState.themeState = ElementTheme::DEFAULT;
    ...
    for (auto& e : internalEdges) {
        ...
        eState.thickness = 4.0f; eState.opacity = 1.0f; eState.themeState = ElementTheme::DEFAULT;
...
int MST::findSet(int i, std::vector<int>& parent) {
...
void MST::GenerateMST() {
    ...
    SetCode({
        ...
        "  for (Edge e : edges) {",
        "    int rootU = findSet(e.u);",
        "    int rootV = findSet(e.v);",
        "    if (rootU != rootV) {",
        "      parent[rootU] = rootV;",
    ...
    for(auto& e : nextEdges) { e.themeState = ElementTheme::DEFAULT; e.opacity = 0.2f; }
    PushNewStep(nextNodes, nextEdges, 1);

    std::vector<MEdge> sortedEdges = internalEdges;
    std::sort(sortedEdges.begin(), sortedEdges.end(), [](const MEdge& a, const MEdge& b) {
        return a.weight < b.weight;
    });

    std::vector<int> parent(internalNodes.size());
    for (size_t i = 0; i < parent.size(); i++) parent[i] = i;
    PushNewStep(nextNodes, nextEdges, 2);

    for (MEdge e : sortedEdges) {
        int vIdx = e.visualIndex;
        nextEdges[vIdx].opacity = 1.0f;
        nextEdges[vIdx].themeState = ElementTheme::HIGHLIGHT; 
        PushNewStep(nextNodes, nextEdges, 3);
        
        nextNodes[e.u].themeState = ElementTheme::HIGHLIGHT;
        nextNodes[e.v].themeState = ElementTheme::HIGHLIGHT;
        ...
        if (rootU != rootV) {
            ...
            nextEdges[vIdx].themeState = ElementTheme::SUCCESS;
            nextNodes[e.u].themeState = ElementTheme::SUCCESS;
            nextNodes[e.v].themeState = ElementTheme::SUCCESS;
            PushNewStep(nextNodes, nextEdges, 8);
        } else {
            nextEdges[vIdx].themeState = ElementTheme::ERROR;
            PushNewStep(nextNodes, nextEdges, 6); 
            nextEdges[vIdx].opacity = 0.05f; 
        }

        for(size_t i = 0; i < parent.size(); i++) {
            if(nextNodes[i].themeState == ElementTheme::HIGHLIGHT) nextNodes[i].themeState = ElementTheme::SUCCESS;
        }
...
void MST::Solve(const Palette &colors) {
    ...
    else if (typeQuery == 1) { GenerateMST(); }
    Draw(colors);
}

void MST::Draw(const Palette &colors) {  
    ...
    DrawTextEx(titleFont, "MINIMUM SPANNING TREE", {319 + 32, 85}, 32, 0.8f, colors.textPrimary);
    ...
    if (!AnimationEdges.empty() && !AnimationNodes.empty()) {
        if (curIndex >= AnimationEdges.size()) {
            for (size_t i = 0; i < currentEdges.size(); i++) {
                ...
                if(finalState.v.themeState == ElementTheme::HIGHLIGHT) c = colors.primaryAcc;
                if(finalState.v.themeState == ElementTheme::SUCCESS) c = colors.success;
                if(finalState.v.themeState == ElementTheme::ERROR) c = colors.error;

                Vector2 start = finalState.v.startPos;
                Vector2 end = finalState.v.endPos;
                ...
                DrawCustomBezierQuad(start, end, controlPos, finalState.v.thickness, Fade(c, finalState.v.opacity));
                Vector2 weightPos = { 0.25f*start.x + 0.5f*controlPos.x + 0.25f*end.x, 0.25f*start.y + 0.5f*controlPos.y + 0.25f*end.y };
                DrawCircle(weightPos.x, weightPos.y, 14, colors.surface);
                DrawTextEx(miniTitleFont, std::to_string(internalEdges[i].weight).c_str(), {weightPos.x - 8, weightPos.y - 10}, 18, 1.0f, Fade(c, finalState.v.opacity));
            }
        } else {
            for (size_t i = 0; i < AnimationEdges[curIndex].size(); i++) {
                auto& step = AnimationEdges[curIndex][i];
                Color c = colors.textPrimary;
                if(step.v.themeState == ElementTheme::HIGHLIGHT) c = colors.primaryAcc;
                if(step.v.themeState == ElementTheme::SUCCESS) c = colors.success;
                if(step.v.themeState == ElementTheme::ERROR) c = colors.error;
                
                float currentOpacity = step.u.opacity + (step.v.opacity - step.u.opacity) * TotalTime;
                
                Vector2 start = step.u.startPos;
                Vector2 end = step.u.endPos;
                Vector2 dir = { end.x - start.x, end.y - start.y };
                float len = sqrt(dir.x*dir.x + dir.y*dir.y);
                Vector2 controlPos = { (start.x + end.x)/2.0f, (start.y + end.y)/2.0f };
                if (len > 0) {
                ...
                DrawCustomBezierQuad(start, end, controlPos, step.u.thickness, Fade(c, currentOpacity));
                Vector2 weightPos = { 0.25f*start.x + 0.5f*controlPos.x + 0.25f*end.x, 0.25f*start.y + 0.5f*controlPos.y + 0.25f*end.y };
                DrawCircle(weightPos.x, weightPos.y, 14, colors.surface);
                DrawTextEx(miniTitleFont, std::to_string(internalEdges[i].weight).c_str(), {weightPos.x - 8, weightPos.y - 10}, 18, 1.0f, Fade(c, currentOpacity));
            }
        }
    ...
    if (isInitDialogOpen) {
        ...
        DrawTextEx(titleFont, "Input Graph Data", {dialogX + 20, dialogY + 20}, 30, 1.0f, colors.textPrimary);
        ...
        if (isRandomHovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            for (int i = 1; i < n; i++) {
                int u = rand() % i;
                int w = rand() % 90 + 10;
                s += std::to_string(u) + " " + std::to_string(i) + " " + std::to_string(w) + "\n";
            }
            int extra = rand() % n;
            for (int i = 0; i < extra; i++) {
                int u = rand() % n; int v = rand() % n;
                if (u != v) s += std::to_string(u) + " " + std::to_string(v) + " " + std::to_string(rand() % 90 + 10) + "\n";
            }
        ...
        DrawTextEx(miniTitleFont, "Format: Line 1 = n (Vertices), Next Lines = u v w.  File drag-and-drop supported.", { dialogX + 20, dialogY + 410 }, 14, 1.0f, colors.textSecondary);
        ...
        if ((isOkHovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))) {
            ...
            if (tokens.size() > 0) {
                ...
                std::vector<MEdge> customEdges;
                int idx = 0;
                for(size_t i = 1; i + 2 < tokens.size(); i += 3) {
                    int u = tokens[i]; int v = tokens[i+1]; int w = tokens[i+2];
                    if (u < n && v < n && u != v) {
                        bool dup = false;
                        for(auto& e : customEdges) {
                            if ((e.u == u && e.v == v) || (e.u == v && e.v == u)) { dup = true; break; }
                        }
                        if (!dup) customEdges.push_back({u, v, w, idx++});
                    }
                }
...
std::string MST::FloatToString(float value) { char buffer[10]; snprintf(buffer, sizeof(buffer), "%.1fx", value); return std::string(buffer); }